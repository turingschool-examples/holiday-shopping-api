...
app.locals = {
  title: 'Holiday Shopping List API',
  items
}
...
app.post('/items', (req, res) => {
  let requiredParams = ['id', 'recipient', 'name', 'priceInDollars'];
...
function checkCorrectDataType(requiredParams, newData, response) {
  ...
  if (typeof newData[requiredParams[1]] !== 'string') {
    return response.status(422).json({
      message: `Invalid format: recipient must be a string`
    });
  }

  if (typeof newData[requiredParams[2]] !== 'string') {
    return response.status(422).json({
      message: `Invalid format: name must be a string`
    });
  }

  if (typeof newData[requiredParams[3]] !== 'number') {
    return response.status(422).json({
      message: `Invalid format: priceInDollars must be a number`
    });
  }
}
